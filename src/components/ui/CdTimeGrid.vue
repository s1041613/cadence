...
// Range 6:00-23:00 (START=6, END=23). Gutter 50px. Hour lines and column dividers use
// --cd-ink-wash-line; the header and all-day underlines use --cd-ink-wash-strong, which reads as
// a section boundary rather than a grid rule. Sticky header z-index 6, all-day bar always
...
.cd-time-grid__header {
  ...
  z-index: 6;
  background: var(--cd-surface-raised);
  border-bottom: 1px solid var(--cd-ink-wash-strong);
}
...
.cd-time-grid__header-num {
  font: 700 21px var(--cd-font-mono);
...
.cd-time-grid__allday-cell {
  ...
  gap: 2px;
  padding: 4px 4px 3px;
  border-left: 1px solid var(--cd-ink-wash-line);
}

.cd-time-grid__allday-item {
  border-left: 3px solid;
  border-radius: 5px;
  font: 700 8.5px var(--cd-font-ui);
  padding: 2px 6px;
}
...
.cd-time-grid__hour-line {
  ...
  left: 0;
  right: 0;
  border-top: 1px solid var(--cd-ink-wash-line);
}
