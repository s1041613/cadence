...
「開場畫面 logo 太大了！比例不對」「要在正中心、上下左右對稱」「有辦法做動畫嗎」。
...
- 標記寬 785 px = **螢幕寬的 65%**。那是滿版插圖，不是開場標記 →
  `min(72vmin, 320px)` 改成 `min(20vmin, 94px)`（402pt 手機上約 80pt）。
  路徑是 65% → 44% → 32%，每一階都在真機上看過還嫌大，最後並排比較 32 / 28 / 25 /
  20 之後定在 20%。我對 25% 以下提過保留（右下小菱形、左邊小十字在 20% 時只剩約
  4pt，眨起來可能看不出是星星），使用者看過 1:1 放大後仍選 20%。
- 標記**本來就置中**：它的方塊中心 y=1404，頁面中心也是 y=1404，同一個像素。
  ...
  因此 `@media (display-mode: standalone)` 下把標記上提 28px（約現代 iPhone
  54–62pt 狀態列的一半）；瀏覽器分頁裡頁面本身就是使用者看到的框，不上提。

圖檔四邊都是貼著筆畫裁的，所以置中方塊＝置中畫面內容，左右等邊、上下等邊。

## 動畫

| 檔案 | 動作 |
| --- | --- |
| `public/splash-core.png` | 新增，156 KB。日曆本體＋尾巴 |
| `public/splash-sparks.png` | 新增，43 KB。六顆星星（粉紅四角星、兩個粉紅十字、深色十字、大小兩顆深色菱形） |
| `public/splash-mark.png` | 刪除。已被上面兩層取代（202 KB → 199 KB，總量還略減） |
| `index.html` | 兩層疊圖 + keyframes + 每顆星星一個元素 |
| `src/composables/use-app-splash.ts` | `MIN_MS` 700 → 1200 |

兩層都畫在同一張 768×663 畫布上，所以不管方塊被縮到多大，疊起來都對得準
（驗證過：兩層合成回去與原圖逐像素相同）。

**每顆星星各自的節奏，但只多一個檔案**：五個元素都用同一張 `splash-sparks.png`，
各自 `clip-path: inset()` 只露出自己那顆、`transform-origin` 設在自己的中心
（所以縮放是原地縮，不是往畫布中心跑）。六顆星星的框互不重疊 —— 右下那兩顆菱形
的框會互相吃到，所以合成一組一起眨。已用程式檢查過每個裁切框內沒有別顆的像素。

| 時間 | 畫面 |
| --- | --- |
| 0.06s | 日曆淡入、上浮 10px、scale .97 → 1（0.56s） |
| 0.24s | 第一顆星星彈出（scale .3 → 1，回彈曲線），之後每 0.09s 一顆 |
| 1.02s | 第五顆落定，全部進入呼吸迴圈（opacity 1↔.55、scale 1↔.92，2.6s） |
| ≥1.20s | 資料到齊即淡出（`MIN_MS`） |
| 6.00s / 8.00s | 兩層保險絲，原封不動 |

呼吸迴圈是故意做成無限的：冷啟動慢的時候，畫面不會凍成一張靜止圖。

`prefers-reduced-motion: reduce` 時 `#cd-splash *` 全部 `animation: none`，直接顯示
最終狀態。選的是子元素，容器上的 fail-open 不受影響 —— 它是安全計時器不是效果。

## 代價

`MIN_MS` 700 → 1200：**每次冷啟動都多等 0.5s**，換動畫播得完。要改回去就只調這個數字，
動畫本身不用動（會被中途淡出截掉，但截在哪一拍都不難看）。
